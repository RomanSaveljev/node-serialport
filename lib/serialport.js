 ...
 * @type {object}
 * @property {function} [ByteLength] is a transform stream that emits data each time a byte sequence is received.
 * @property {function} [Delimiter] is a transform stream that emits data as a buffer after a specific number of bytes are received.
 * @property {function} [ReadLine] is a transform stream that emits data after a newline delimiter is received.
 * @example
...
var parser = new Delimiter({delimiter: new Buffer('EOL')});
port.pipe(parser);
parser.on('data', console.log);
```
